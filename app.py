...
# FIXED: Definition explicitly accepts 3 arguments
...
def generate_briefing(ctx, s_data, m_data):
    try:
        ...
        analysis_payload = {
            ...
            "metrics": {"net_profit": float(ctx['total_net_profit']), "revenue": float(s_data['total_sales']), "ad_spend": float(m_data['total_spend']), "blended_roas": float(ctx['blended_mer']), "new_customers": int(s_data['new_orders']), "ncpa": float(ctx['ncpa']), "aov": float(s_data['aov'])},
    ...
    with st.spinner("Syncing & Analyzing..."):
        ...
        try:
            ...
            if shop_data and meta_data:
                ...
                # FIX: FLATTEN HTML TO AVOID MARKDOWN PARSER ISSUES
                ...
                briefing_html = (
                    ...
                    f"<div style='margin-bottom: 10px;'><div style='color: #00E676; font-size: 12px; font-weight: 600; margin-bottom: 4px;'>WINS</div>{wins_html}</div>"
                    f"<div style='margin-bottom: 10px;'><div style='color: #FF3D00; font-size: 12px; font-weight: 600; margin-bottom: 4px;'>WARNINGS</div>{warn_html}</div>"
                    ...
                    if view_mode == "Grid":
                        ...
                        for i, ad in enumerate(ads):
                            with cols[i % 3]:
                                ...
                                # FLATTENED HTML TO FIX FORMATTING
                                html = (
                                    f'<a href="{link}" target="_blank" class="ad-link"><div class="ad-card"><div class="ad-image-container">'
                                    f'<div class="ad-bg" style="background-image: url(\'{img_src}\');"></div>'
                                    f'<img src="{img_src}" class="ad-image" onerror="this.src=\'https://via.placeholder.com/300x300/222/888?text=Video+Ad\'">'
                                    f'<div class="ad-link-icon">↗</div><div class="ad-badge-top" style="background-color: {badge_color}">{roas_val}x</div></div>'
                                    f'<div class="ad-footer"><div class="ad-title" title="{ad["name"]}">{ad["name"]}</div>'
                                    f'<div class="context-tag" title="Campaign: {ad["campaign"]}">{ad["campaign"]}</div>'
                                    f'<div class="grid-stats">'
                                    f'<div class="stat-box">Spend <div class="text-val">${ad["spend"]:,.0f}</div></div>'
                                    f'<div class="stat-box" style="text-align:right;">Rev <div class="text-val">${ad["revenue"]:,.0f}</div></div>'
                                    f'<div class="stat-box">Sales <div class="text-val">{ad["purchases"]}</div></div>'
                                    f'<div class="stat-box" style="text-align:right;">CPA <div class="text-val">${ad["cpa"]:.2f}</div></div>'
                                    f'<div class="stat-box">CTR <div class="text-val">{ad["ctr"]:.2f}%</div></div>'
                                    f'<div class="stat-box" style="text-align:right;">CPM <div class="text-val">${ad["cpm"]:.2f}</div></div></div>'
                                    f'<div style="font-size:10px; color:#555; margin-top:8px; text-align:center;">Live for {ad["days_live"]} days</div></div></div></a>'
                                )
                        ...
                        for ad in ads:
                            ...
                            # FLATTENED HTML TO FIX FORMATTING
                            html = (
                                f'<a href="{link}" target="_blank" class="ad-link"><div class="list-row">'
                                f'<img src="{img_src}" class="list-img" onerror="this.src=\'https://via.placeholder.com/100x100/222/888?text=Ad\'">'
                                f'<div class="list-content"><div class="list-info"><div style="font-weight:600; color:#fff; font-size:13px; margin-bottom:4px;">{ad["name"]}</div>'
                                f'<div style="font-size:11px; color:#666;">{ad["campaign"]} • Live {ad["days_live"]}d</div></div>'
                                f'<div class="list-metrics"><div>Spend <div class="list-val">${ad["spend"]:,.0f}</div></div>'
                                f'<div>Sales <div class="list-val">{ad["purchases"]}</div></div>'
                                f'<div>CPA <div class="list-val">${ad["cpa"]:.2f}</div></div>'
                                f'<div>ROAS <div class="list-val" style="color:{badge_color}">{roas_val}x</div></div></div></div></div></a>'
                            )
                            st.markdown(html, unsafe_allow_html=True)
