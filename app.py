    ...
    try:
        ...
        for r in records:
            if "|||" in str(r["content"]):
                ...
                r["content"] = parts[0]
                try:
                    r["suggestions"] = json.loads(parts[1])
                except:
                    r["suggestions"] = []
        return records
    except Exception:
...
def save_memory(role, content, suggestions=None):
    try:
        ...
        new_row = pd.DataFrame(
            [{"timestamp": datetime.now().isoformat(), "role": role, "content": save_content}]
        )
...
def get_product_costs(domain, token, variant_ids):
    ...
    try:
        ...
        for chunk in chunks:
            ...
            url = (
                f"https://{domain}/admin/api/2023-10/variants.json?"
                f"ids={ids_str}&fields=id,inventory_item_id"
            )
            ...
            if res.status_code == 200:
                ...
                if inv_ids:
                    ...
                    url2 = (
                        f"https://{domain}/admin/api/2023-10/inventory_items.json?"
                        f"ids={inv_str}&fields=id,cost"
                    )
                    ...
                    if res2.status_code == 200:
                        ...
                        for item in items:
                            ...
                            if vid:
                                cost_map[vid] = float(item["cost"] or 0)
    except Exception:
        pass
    return cost_map
...
def fetch_shopify_data(domain, token, fallback_margin, start_date, end_date):
    try:
        ...
        url = (
            f"https://{domain}/admin/api/2023-10/orders.json?"
            f"status=any&created_at_min={start_iso}&created_at_max={end_iso}"
            "&limit=250&fields=id,created_at,total_price,line_items,customer"
        )
        headers = {"X-Shopify-Access-Token": token}
        all_orders = []
        while url:
            ...
            time.sleep(0.3)
            if res.status_code != 200:
                return None, f"Shopify Error {res.status_code}"
            data = res.json()
        ...
        for o in all_orders:
            ...
            if "customer" in o and o["customer"]:
                if int(o["customer"].get("orders_count", 1)) > 1:
                    is_new = False
                try:
                    ord_time = datetime.fromisoformat(
                        o["created_at"].replace("Z", "+00:00")
                    )
                    cust_time = datetime.fromisoformat(
                        o["customer"]["created_at"].replace("Z", "+00:00")
                    )
                    if (ord_time - cust_time).total_seconds() > 43200:
                        is_new = False
                except:
                    pass
        ...
        return {
            ...
            "top_products": sorted(
                prod_sales.items(), key=lambda x: x[1], reverse=True
            )[:5],
            "aov": total_rev / len(all_orders) if len(all_orders) > 0 else 0,
            "order_count": len(all_orders),
        }, None
    except Exception as e:
        return None, f"Shopify Crash: {e}"
...
def fetch_ad_creatives_batch(token, ad_ids):
    ...
    for chunk in chunks:
        try:
            ids_str = ",".join(chunk)
            url = (
                "https://graph.facebook.com/v17.0/?ids="
                f"{ids_str}&fields=creative{{image_url,thumbnail_url,"
                "object_story_spec,asset_feed_spec,effective_object_story_id,"
                "instagram_permalink_url}}&access_token={token}"
            )
            ...
            if res.status_code == 200:
                ...
                for ad_id, val in data.items():
                    ...
                    try:
                        ...
                        img = spec.get("link_data", {}).get("full_picture") or spec.get(
                            "link_data", {}
                        ).get("picture")
                        if not img:
                            img = spec.get("photo_data", {}).get("url")
                        if not img:
                            img = spec.get("video_data", {}).get("image_url")
                    except:
                        pass
                    if not img:
                        try:
                            ...
                            pass
                    if not img:
                        img = creative.get("image_url") or creative.get(
                            "thumbnail_url"
                        )
    ...
    try:
        ...
        cmp_res = requests.get(
            ...
            params={
                ...
                "level": "campaign",
                "fields": (
                    "campaign_name,spend,clicks,impressions,actions,"
                    "action_values,cpm,ctr,cpc"
                ),
                "limit": 100,
            },
        )
        ...
        ad_res = requests.get(
            ...
            params={
                ...
                "level": "ad",
                "fields": (
                    "ad_id,ad_name,adset_name,campaign_name,created_time,spend,"
        ...
        if cmp_res.status_code != 200:
            return None, f"Meta Campaign Error: {cmp_res.text}"
        if daily_res.status_code != 200:
            return None, f"Meta Daily Error: {daily_res.text}"
        if ad_res.status_code != 200:
            return None, f"Meta Ad Error: {ad_res.text}"
        ...
        daily_spend = [
            {"date": d["date_start"], "spend": float(d["spend"])} for d in daily_data
        ]
        ...
        for c in cmp_data:
            ...
            sales_val = (
                sum(float(a["value"]) for a in actions if a["action_type"] == "purchase")
                if actions
                else 0
            )
            campaigns.append(
        ...
        for a in ad_data:
            ...
            if spend > 0 or int(a.get("impressions", 0)) > 10:
                ...
                purchases = sum(
                    float(x["value"])
                    for x in actions
                    if x["action_type"] == "purchase"
                )
                revenue = sum(
                    float(x["value"])
                    for x in action_values
                    if x["action_type"] == "purchase"
                )
                ad_id = a.get("ad_id") or a.get("id")
                created = datetime.strptime(
                    a.get("created_time", datetime.now().strftime("%Y-%m-%d")),
                    "%Y-%m-%d",
                )
    ...
    try:
        ...
        analysis_payload = {
            ...
            "metrics": {
                ...
                "ncpa": float(ctx["ncpa"]),
                "aov": float(s_data["aov"]),
            },
            "top_products": [str(p[0]) for p in s_data["top_products"]],
            "campaigns": m_data["campaign_df"].head(5).to_dict("records"),
        }
        system_prompt = (
            'You are an elite eCommerce Analyst who specialized in Shopify analytics '
            'and Meta Ads. Analyze the data from Shopify and Meta Ads and return a '
            'JSON object with exactly these keys: {"headline": "A short, punchy '
            '1-sentence summary.", "wins": ["Bullet 1", "Bullet 2"], "warnings": '
            '["Bullet 1", "Bullet 2"], "action_plan": "One clear strategic '
            'recommendation.", "suggested_questions": ["Question 1", "Question 2", '
            '"Question 3"]} Do not include markdown formatting. Return RAW JSON only.'
        )
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": json.dumps(analysis_payload, default=safe_serialize),
                },
            ],
...
if "last_synced_dates" not in st.session_state:
...
with header_col2:
    ...
    if preset == "Last 7 Days":
        s_d, e_d = today - timedelta(days=7), today
    elif preset == "Last 30 Days":
        s_d, e_d = today - timedelta(days=30), today
    elif preset == "This Month":
        s_d, e_d = today.replace(day=1), today
    ...
    date_range = st.date_input(
        "Custom Range", value=(s_d, e_d), label_visibility="collapsed"
    )
    if len(date_range) == 2:
        s_d, e_d = date_range
...
def run_sync_logic():
    with st.spinner("Syncing & Analyzing..."):
        ...
        try:
            ...
            shop_data, s_err = fetch_shopify_data(
                s_domain, s_token, margin_pct, s_d, e_d
            )
            meta_data, m_err = fetch_meta_data(m_token, m_id, s_d, e_d)
            if s_err:
                st.session_state.logs.append(s_err)
            if m_err:
                st.session_state.logs.append(m_err)
            if shop_data and meta_data:
                df_s = shop_data["daily_df"]
                df_m = meta_data["daily_spend_df"]
                if not df_s.empty and not df_m.empty:
                    df_merged = (
                        pd.merge(df_s, df_m, on="date", how="outer")
                        .fillna(0)
                        .sort_values("date")
                    )
                    df_merged["gross_profit"] = df_merged["sales"] - df_merged["cogs"]
                    df_merged["net_profit"] = df_merged["gross_profit"] - df_merged[
                        "spend"
                    ]
                    df_merged["mer"] = df_merged.apply(
                        lambda row: row["sales"] / row["spend"]
                        if row["spend"] > 0
                        else 0,
                        axis=1,
                    )
                ...
                blended_mer = (
                    shop_data["total_sales"] / meta_data["total_spend"]
                    if meta_data["total_spend"] > 0
                    else 0
                )
                ncpa = (
                    meta_data["total_spend"] / shop_data["new_orders"]
                    if shop_data["new_orders"] > 0
                    else 0
                )
                ...
                ctx = {
                    ...
                    "date_range": f"{s_d} to {e_d}",
                    "blended_mer": blended_mer,
                    "ncpa": ncpa,
                    "roas": shop_data["total_sales"] / meta_data["total_spend"]
                    if meta_data["total_spend"] > 0
                    else 0,
                }
                ...
                wins_html = "".join(
                    [
                        f"<div class='briefing-item'><span class='briefing-icon'>✅</span>{x}</div>"
                        for x in briefing.get("wins", [])
                    ]
                )
                warn_html = "".join(
                    [
                        f"<div class='briefing-item'><span class='briefing-icon'>⚠️</span>{x}</div>"
                        for x in briefing.get("warnings", [])
                    ]
                )
                briefing_html = (
                    ...
                    "<div style='color: #00E676; font-size: 12px; font-weight: 600; "
                    "margin-bottom: 4px;'>WINS</div>"
                    f"{wins_html}</div>"
                    "<div style='margin-bottom: 10px;'>"
                    "<div style='color: #FF3D00; font-size: 12px; font-weight: 600; "
                    "margin-bottom: 4px;'>WARNINGS</div>"
                    f"{warn_html}</div>"
                    "<div style='margin-top: 12px; padding-top: 10px; "
                    "border-top: 1px solid #333;'>"
                    "<div style='color: #0A84FF; font-size: 12px; "
                    "font-weight: 600;'>RECOMMENDATION</div>"
                    "<div style='font-size: 13px; color: #ccc;'>"
                    f"{briefing.get('action_plan')}</div></div></div>"
                )
                ...
                save_memory(
                    "assistant",
                    briefing_html,
                    briefing.get("suggested_questions", []),
                )
            ...
            st.session_state.logs.append(f"Config Error: {e}")
...
<style>
    ...
    .stButton button {{ border: 1px solid #333; background: #1a1a1a; color: #ddd; border-radius: 20px; font-size: 12px; padding: 4px 12px; margin-right: 5px; }}
    ...
    with st.container(height=850, border=False):
        if "context" in st.session_state:
            ...
            tab1, tab2, tab3, tab4 = st.tabs(
                ["Growth (New vs Ret)", "Profit Chart", "Creative Gallery", "Campaigns"]
            )
            with tab1:
                ...
                if not df.empty:
                    ...
                    fig.add_trace(
                        go.Bar(
                            ...
                            y=df["net_profit"],
                            name="Net Profit",
                            marker_color=df["net_profit"].apply(
                                lambda x: "#00C853" if x >= 0 else "#FF3D00"
                            ),
                        )
                    )
                    fig.add_trace(
            ...
            with tab3:
                ...
                with col_sort:
                    sort_mode = st.selectbox(
                        "Sort",
                        ["Highest Spend", "Best ROAS", "Most Sales"],
                        label_visibility="collapsed",
                    )
                ads = m_data.get("gallery_ads", [])
                if ads:
                    if sort_mode == "Highest Spend":
                        ads = sorted(ads, key=lambda x: x["spend"], reverse=True)
                    elif sort_mode == "Best ROAS":
                        ads = sorted(ads, key=lambda x: x["roas"], reverse=True)
                    elif sort_mode == "Most Sales":
                        ads = sorted(ads, key=lambda x: x["purchases"], reverse=True)
                    if view_mode == "Grid":
                        cols = st.columns(3)
                        for i, ad in enumerate(ads):
                            with cols[i % 3]:
                                img_src = ad.get("image_url") or (
                                    "https://via.placeholder.com/300x300/222/888"
                                    "?text=No+Image"
                                )
                                roas_val = ad["roas"]
                                badge_color = (
...
""",
                        ...
                        for ad in ads:
                            img_src = ad.get("image_url") or (
                                "https://via.placeholder.com/100x100/222/888?text=Img"
                            )
                            roas_val = ad["roas"]
                            badge_color = (
                                "#00E676"
...
<a href="{link}" target="_blank" class="ad-link">
  <div class="list-row">
    <img src="{img_src}" class="list-img"
         onerror="this.src='https://via.placeholder.com/100x100/222/888?text=Ad'">
    <div class="list-content">
      <div class="list-info">
        ...
        <div style="font-size:11px; color:#666;">
          {ad['campaign']} • Live {ad['days_live']}d
        </div>
      </div>
...
""",
                    ...
                    st.info("No active creatives found in this date range.")
            with tab4:
                st.dataframe(
...
if run_ai:
    try:
        ...
        if "context" in st.session_state:
            ...
            ads_txt = "\n".join(
                [
                    f"{a['name']}: {a['roas']}x ROAS (${a['spend']})"
                    for a in m_data["gallery_ads"][:10]
                ]
            )
            context_str = (
                "OVERVIEW:\n"
                f"Blended MER: {ctx['blended_mer']:.2f}x\n"
                f"Net Profit: ${ctx['total_net_profit']:,.2f}\n"
                f"Revenue: ${s_data['total_sales']}\n"
                f"Orders: {s_data['order_count']}\n"
                f"Ad Spend: ${m_data['total_spend']}\n"
                f"ROAS: {ctx['roas']:.2f}x\n\n"
                "NEW vs RET:\n"
                f"New Rev: ${s_data['new_cust_rev']}\n"
                f"Returning: ${s_data['ret_cust_rev']}\n\n"
                "TOP ADS:\n"
                f"{ads_txt}\n\n"
                "CAMPAIGNS:\n"
                f"{m_data['campaign_df'].to_string(index=False)}"
            )
